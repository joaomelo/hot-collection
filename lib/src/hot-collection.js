...
class HotCollection {
  ...
  __ignite () {
    ...
    query.onSnapshot(snapshot => {
      ...
      publish(this.STATE_UPDATED, this.state.items);
    });
  }

  subscribe (callback) {
    return subscribe(this.STATE_UPDATED, callback, true);
  }
  ...
  getItems () {
    ...
    const promise = new Promise((resolve, reject) => {
      if (this.state.status === STATUSES.IDLE) {
        ...
        const unsub = subscribe(this.STATE_UPDATED, items => {
          unsub();
          resolve(items);
        });
      }
