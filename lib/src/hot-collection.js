...
class HotCollection {
  constructor (collection, options = { adapter: 'in-memory' }) {
    ...
    // a collectionUpdatedSignal observer can always check if
    // the payload equals null to know if the hotcollection
    // didn't load any database data snapshot yet
    const collectionUpdatedSignal = new BehaviorSubject(null);
